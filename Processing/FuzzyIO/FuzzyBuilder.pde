...
class FuzzyBuilder {
  ...
  final private float DEFAULT_CANTILEVER_ALLOWANCE = 0.5;
  ...
  public Development build(SettingGroup settings) {
    ...
    try {
      ...
      if (settings.settingValues.size() >= 2) {
        voxelHeight = parseFloat(settings.settingValues.get(0).value);
        cantileverAllowance = parseFloat(settings.settingValues.get(1).value) / 100f;
      } else {
